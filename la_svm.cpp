...
/* Hacky (not very elegant) way to implement batches */
vector<int> chunks;
...
void exit_with_help()
{
    fprintf(stdout,
            ...
            "-z chunks: simulate streaming batches. Ex: -z 10 20 30, the model is first trained with the samples ranging from 0 to 10 (as if there were no other samples), then with 10-20, and then finally 20-30. \n");
    exit(1);
}
...
out:
    ...
    for (i = 0; i < msz; i++)
    {
        ...
        while (1)
        {
            ...
            if (write == 2)
                lasvm_sparsevector_set(X[splitpos - 1], index, value);
            if (index > max_index)
                max_index = index;
        }
    out2:
...
void train_online_batch(char *model_file_name)
{
    ...
    for (; current_chunk < chunks.size(); current_chunk++)
    {
...
int main(int argc, char **argv)
{
    ...
    train_online_batch(model_file_name);
